...
public class JSoupFilters extends JSoupFilter implements JSONResource {
	...
	/**
	 * Loads and configure the JSoupFilters based on the storm config if there is
	 * one otherwise returns an empty JSoupFilter.
	 **/
	...
	public static JSoupFilters fromConf(Map stormConf) {
		...
		if (StringUtils.isNotBlank(parseconfigfile)) {
			try {
				...
				String message = "Exception caught while loading the JSoupFilters from " + parseconfigfile;
				LOG.error(message);
	...
	public void configure(Map stormConf, JsonNode filtersConf) {
		List<JSoupFilter> list = Configurable.configure(stormConf, filtersConf, JSoupFilter.class,
