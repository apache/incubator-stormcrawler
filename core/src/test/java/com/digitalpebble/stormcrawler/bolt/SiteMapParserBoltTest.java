...
public class SiteMapParserBoltTest extends ParsingTester {
	...
	public void testSitemapParsingWithAllExtensions() throws IOException {
		...
		parserConfig.put("sitemap.extensions", Arrays.asList(Extension.MOBILE.name(), Extension.NEWS.name(),
				Extension.LINKS.name(), Extension.VIDEO.name(), Extension.IMAGE.name()));
		prepareParserBolt("test.parsefilters.json", parserConfig);
	...
	private void assertNewsAttributes(Metadata metadata) {
		long numAttributes = metadata.keySet().stream().filter(key -> key.startsWith(Extension.NEWS.name() + "."))
				.count();
		...
		Assert.assertArrayEquals(new String[] { "PressRelease", "Blog" },
				metadata.getValues(Extension.NEWS.name() + "." + "genres"));
		Assert.assertEquals("2008-12-23T00:00Z",
				metadata.getFirstValue(Extension.NEWS.name() + "." + "publication_date"));
		Assert.assertEquals("Companies A, B in Merger Talks",
				metadata.getFirstValue(Extension.NEWS.name() + "." + "title"));
		Assert.assertArrayEquals(new String[] { "business", "merger", "acquisition", "A", "B" },
				metadata.getValues(Extension.NEWS.name() + "." + "keywords"));
		Assert.assertArrayEquals(new String[] { "NASDAQ:A", "NASDAQ:B" },
				metadata.getValues(Extension.NEWS.name() + "." + "stock_tickers"));
	}

	private void assertImageAttributes(Metadata metadata) {
		long numAttributes = metadata.keySet().stream().filter(key -> key.startsWith(Extension.IMAGE.name() + "."))
				.count();
		...
		Assert.assertEquals("http://example.com/photo.jpg",
				metadata.getFirstValue(Extension.IMAGE.name() + "." + "loc"));
		Assert.assertEquals("Example photo shot in Limerick, Ireland",
				metadata.getFirstValue(Extension.IMAGE.name() + "." + "title"));
		Assert.assertEquals("https://creativecommons.org/licenses/by/4.0/legalcode",
	...
	private void assertLinksAttributes(Metadata metadata) {
		long numAttributes = metadata.keySet().stream().filter(key -> key.startsWith(Extension.LINKS.name() + "."))
				.count();
		Assert.assertEquals(3, numAttributes);
		Assert.assertEquals("alternate", metadata.getFirstValue(Extension.LINKS.name() + "." + "params.rel"));
		Assert.assertEquals("http://www.example.com/english/",
	...
	private void assertVideoAttributes(Metadata metadata) {
		...
		Assert.assertEquals("Alkis shows you how to get perfectly done steaks every time",
				metadata.getFirstValue(Extension.VIDEO.name() + "." + "description"));
		Assert.assertEquals("http://www.example.com/video123.flv",
				metadata.getFirstValue(Extension.VIDEO.name() + "." + "content_loc"));
		Assert.assertEquals("http://www.example.com/videoplayer.swf?video=123",
		...
		// "." + "duration"));
		Assert.assertEquals("2009-11-05T19:20:30+08:00",
		...
		Assert.assertEquals("2007-11-05T19:20:30+08:00",
		...
		Assert.assertArrayEquals(new String[] { "sample_tag1", "sample_tag2" },
				metadata.getValues(Extension.VIDEO.name() + "." + "tags"));
		Assert.assertArrayEquals(new String[] { "IE", "GB", "US", "CA" },
				metadata.getValues(Extension.VIDEO.name() + "." + "allowed_countries"));
		Assert.assertEquals("http://cooking.example.com",
				metadata.getFirstValue(Extension.VIDEO.name() + "." + "gallery_loc"));
		Assert.assertEquals("value: 1.99, currency: EUR, type: own, resolution: null",
		...
		Assert.assertEquals("http://www.example.com/users/grillymcgrillerson",
	...
	private void assertMobileAttributes(Metadata metadata) {
		long numAttributes = metadata.keySet().stream().filter(key -> key.startsWith(Extension.MOBILE.name() + "."))
				.count();
