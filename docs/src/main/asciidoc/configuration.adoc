...
| http.credentials.allow.insecure | false | (OkHttp only) If true, send credentials (basic auth, credential headers, cookies) even over connections whose server certificate was not validated, i.e. when `http.trust.everything` is true.
