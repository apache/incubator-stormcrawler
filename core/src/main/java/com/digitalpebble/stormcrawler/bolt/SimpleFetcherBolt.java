...
public class SimpleFetcherBolt extends StatusEmitterBolt {
  ...
  public void prepare(Map stormConf, TopologyContext context, OutputCollector collector) {
    ...
    this.eventCounter =
        context.registerMetric("fetcher_counter", new MultiCountMetric(), metricsTimeBucketSecs);

    this.averagedMetrics =
        context.registerMetric(
            "fetcher_average", new MultiReducedMetric(new MeanReducer()), metricsTimeBucketSecs);
    ...
    this.protocolMDprefix =
        ConfUtils.getString(conf, ProtocolResponse.PROTOCOL_MD_PREFIX_PARAM, protocolMDprefix);
  }
    ...
    try {
      ...
      if (!fromCache && smautodisco) {
        for (String sitemapURL : rules.getSitemaps()) {
          if (rules.isAllowed(sitemapURL)) {
            emitOutlink(input, url, sitemapURL, metadata, SiteMapParserBolt.isSitemapKey, "true");
          }
        }
      }
        ...
        if (timeToWait > 0) {
          ...
          if (maxThrottleSleepMSec != -1 && timeToWait > maxThrottleSleepMSec) {
            collector.emitDirect(
                this.taskID, THROTTLE_STREAM, input, new Values(urlString, metadata));
            collector.ack(input);
      ...
      if (robotsDelay > 0) {
        if (robotsDelay > maxCrawlDelay) {
          if (maxCrawlDelayForce) {
            ...
            LOG.debug(
                "Skipped URL from queue with overlong crawl-delay ({}): {}", robotsDelay, url);
          ...
          LOG.debug(
              "Crawl delay for {} too short ({}), set to fetcher.server.delay", url, robotsDelay);
          delay = crawlDelay;
        } else {
          delay = robotsDelay;
        }
      ...
      response.getMetadata().keySet().stream()
          ...
          .forEach(
              s ->
                  averagedMetrics
                      .scope(s.substring(8))
                      .update(Long.parseLong(response.getMetadata().getFirstValue(s))));
      ...
      if (status.equals(Status.FETCHED)) {
        if (response.getStatusCode() == 304) {
          ...
          collector.emit(
              com.digitalpebble.stormcrawler.Constants.StatusStreamName, input, values4status);
        } else {
          collector.emit(
